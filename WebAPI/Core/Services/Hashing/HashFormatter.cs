...
namespace Core.Services.Hashing
{
    internal class HashFormatter
    {
        ...
        private readonly Dictionary<string, HashAlgorithmName> _algorithmKeys
            = new Dictionary<string, HashAlgorithmName>
            {
                ["alg1"] = HashAlgorithmName.SHA256
            };

        public string Build(HashContext context)
        {
            string algorithmKey = this._algorithmKeys.First(pair => pair.Value == context.HashAlgorithm).Key;
        ...
        public HashContext Parse(string hash)
        {
            ...
            if (hashParts.Length == HASH_FORMAT_PARTS)
            {
                ...
                {
                    HashAlgorithm = this._algorithmKeys.GetValueOrDefault(metaParts[META_FORMAT_ALGORITHM_INDEX]),
