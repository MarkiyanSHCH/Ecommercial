...
namespace Core.Handlers.Hashing
{
    internal static class HashFormatter
    {
        ...
        private static readonly Dictionary<string, HashAlgorithmName> _algorithmKeys
        ...
        public static HashContext Parse(string hash)
        {
            if (string.IsNullOrWhiteSpace(hash))
                throw new ArgumentNullException(nameof(hash), "Hash was not provided.");
            ...
            if (hashParts.Length == HASH_FORMAT_PARTS)
            {
                ...
                if (metaParts.Length != META_FORMAT_PARTS)
                    throw new FormatException($"Hash metadata formatted incorrectly, meta format - {META_FORMAT_TEMPLATE}");
            ...
            throw new FormatException($"Invalid hash format, expecting - {HASH_FORMAT_TEMPLATE}");
        }
    }
}
